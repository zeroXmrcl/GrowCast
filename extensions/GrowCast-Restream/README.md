...
Started by default with `docker compose up`. Captures `/overlay/capture` in Chromium and pushes RTMPS to Twitch. Stays idle until Stream (`/admin/stream`) → Start.

Needs about 1–2 CPU cores and 1 GB RAM.

GrowCast auto-creates `data/restream/capture.token` (Stream page load, Twitch Start, or capture authorize). Optional `GROWCAST_RESTREAM_TOKEN` in `.env.local` overrides that file. Put the Twitch stream key on Stream; it is stored under `data/restream/`, not in the public grow JSON.
