...
Encoder is a Compose sidecar (Chromium + ffmpeg) started with `docker compose up`, not the Next.js process. Stream key lives in `data/restream/twitch.key`, never public grow JSON.
